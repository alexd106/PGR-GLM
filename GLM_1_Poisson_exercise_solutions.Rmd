...
As concisely as possible, write down the research question that can be answered with our analysis. This might seem trivial in this case, but it's often one of the hardest parts of research; going from science, to data, to an actionable research question.
...
For our purposes, the deterministic part we're interested in is what role plant biomass and soil pH plays in determining species richness. If you're comfortable doing so, write down a sensible deterministic element for the model you would run, using statistical notation.

Hint: If you're struggling, come back to this part having run a `glm()`, and use the `summary()` to help guide you.
...
# If you wrote:
# log(lambda) = beta_0 + beta_1 * biomass + beta_2 * pH
# Then you'd be wrong in this case (though maybe not always depend on how we measure pH)
# Here, pH is a categorical variable, labelled as low, mid and high
# As such, we can't multiply, say 1.2, by "mid". Instead, we need to convert
# these words into numbers. The way R and glm() does this is to create so-called
# indicator variables. We could create 3 variables in this case for low, mid and
# high, but we can actually drop one if we say beta_2 and 3 are contrasts, allowing us
# to be a little more efficient in terms of the number of parameters.
```
...
Having thought through the above details, it's now time to bring that information together into a single model. As a reminder, we want to describe the associations between species richness and plant biomass, and soil pH. Given this is the first practical session, we'll break this up for the sake of pedagogy. 

Let's start with species richness explained by biomass. 

Write the equation (not the `R` code) for this model.
...
Hopefully you identified issues that mean we should be very cautious with interpreting this simple version of the model. For now, we'll put these concerns to rest (we will come back to these issues), and use this model as an opportunity to practice making predictions (and therefore answering our research question).
...
9\. We've run a Poisson GLM where `Species` is explained by `Biomass`, but remember that our initial research question sought to describe the relationships between `Species` with both `Biomass` *and* `pH`. This should be the model you wrote back in section 3 (if you skipped that section, go back to it after running this next model). 
...
10\. With every new model we run, we need to diagnose that model. We've already done that once, so let's quickly do it again.
...
We can then use this new *synthetic* dataset to create predictions of species richness using `predict()` function. The `predict()` function takes the model equation (with the estimated parameter values) and combines this with either the original data (if you do not supply a synthetic dataset) or with synthetic data (if you do supply a synthetic dataset). It is doing exactly the same as what you were doing in section 8 - there's no magic here.

`synth_data$pred <- predict(sp.glm2, newdata = synth_data)` 

A hint: we can tell the `predict()` function to use the inverse link function automatically when making the predictions, so we don't need to by setting `type = "response"` or else we could always do this manually by exponentiating our predicted values (note for the tomorrow's lecture, you shouldn't use `type = "response"` if you are going to create 95% CI - more tomorrow). 

Once done, use your new synthetic data and predicted species richness to create a figure that shows the relationships for pH and biomass.
...
# We'll discuss how expand.grid() works tomorrow (and the problem with using it)
# But for now, just know that it creates a suitable "fake" dataset
