...
As concisely as possible, write down the research question that can be answered with our analysis. This might seem trivial in this case, but it's often one of the hardest parts of research; going from a theoretical query to a research question.
...
```{r Q13, eval=TRUE, echo=SOLUTIONS, results=SOLUTIONS, collapse=TRUE, fig.show= ifelse(SOLUTIONS, "asis", "hide")}
...
We can use the same technique we used for the model with just biomass, but now extend it to include the differing levels of pH. In addition, rather than manually writing out the equation (and possibly making mistakes), we can use the `predict()` function to just do it for us. A trick we can use here is to make use of the `expand.grid()` function, to create a new dataset that has a sequence of biomass values, that is replicate for each level of pH. To do so, we need to specify the range of values for both `biomass` and `ph`. We already did this for `biomass`, and it's pretty easy to do for `ph` as well: `c("Low", "Medium", "High")`. With that, we can feed these into `expand.grid()`, as `synth_data <- expand.grid(Biomass = seq(from = min(dat$Biomass), to = max(dat$Biomass), length.out = 5), pH = c("low", "mid", "high"))`. You can now feed this new *synthetic* data into the predict function, while also specifying which model to use to make the prediction, as `synth_data$pred <- predict(sp.glm2, newdata = synth_data)`. A final, final trick; we can tell the `predict()` function to use the inverse link function automatically when making the predictions, so we don't need to by setting `type = "response"`: `(predict(sp.glm2, new_data = synth_data, type = "response")`). Using these predicted values, create a figure to show your predicted relationships for pH and biomass.
...
For those who use, or are interested in using `ggplot2` to create figures, here is how you'd create the equivalent figure using the `ggplot2` package.

```{r ggplot, eval=TRUE, echo=SOLUTIONS, results=SOLUTIONS, collapse=TRUE}
library(ggplot2)

ggplot() +
  geom_point(data = sp, aes(x = Biomass, y = Species, colour = pH)) +
  geom_line(data = synth_data, aes(x = Biomass, y = pred, colour = pH))

# Or with a bit more complexity
ggplot() +
  geom_point(data = sp, aes(x = Biomass, y = Species, colour = pH)) +
  geom_line(data = synth_data, aes(x = Biomass, y = pred, colour = pH)) +
