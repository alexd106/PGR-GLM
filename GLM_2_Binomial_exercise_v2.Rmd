...
* Hints:
  ...
  + One approach to find out if there are categories with very few data, is to count observations per categories of interest.
  + `table()` is a useful way to count the number of observations per category or combinations of categories, e.g. `ObsPerMonthYear<- table(dat$year, dat$mon)` for all combinations of year and month.
  + `plot(ObsPerMonthYear)` returns a "mosaic plot" where the area of each rectangle is proportional to the count.
  + For proportion of time present (the parameter that we're ultimately trying to estimate with the binomial GLM), you could calculate mean presence per category `bla<- tapply(dat$presence, list(dat$GroupOfInterest), mean)`
...
```{r Q6, eval=TRUE, echo=TRUE, results=SOLUTIONS, collapse=TRUE, fig.show= ifelse(SOLUTIONS, "asis", "hide")}
library(car)
vif(PA1)
# No concern.
...
```{r Q6c, eval=TRUE, echo=TRUE, results=SOLUTIONS, collapse=TRUE, fig.show= ifelse(SOLUTIONS, "asis", "hide")}
par(mfrow= c(2, 2))
...
8\. Let’s move on to a richer Binomial (Bernoulli) GLM, with some interactions of interest between categorical time of day, tide angle and day of the year as predictors: `fTide4 + Per2 + Time6 + fTide4:Per2 + fTide4:Time6 + Per2:Time6`. Which individual interactions are implied in this formula? (Hint: if unsure, the summary of the model at the next question will list them). 
...
```{r Q13, eval=TRUE, echo=SOLUTIONS, results=SOLUTIONS, collapse=TRUE, fig.height=8, , fig.width=8}
...
```{r QA1c, eval=TRUE, echo=SOLUTIONS, results=SOLUTIONS, collapse=TRUE, fig.show= ifelse(SOLUTIONS, "asis", "hide")}
# residual analysis:
